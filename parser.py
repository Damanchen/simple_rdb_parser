...
from encodehelpers import STRING_ESCAPE_RAW, apply_escape_bytes, bval
...
class RdbParser(object):
    ...
    # 输出日志
    def log(self, log):
        self._callback.print_some_log(log)
    ...
    def parse_fd(self, fd):
        with fd as f:
            ...
            while True:
                ...
                # 注意 REDIS_RDB_OPCODE_MODULE_AUX = 247 ，这个跟 RDB 文件中 bfkey 前面看到的字符串不符，看到的是 07，而不是 f7
                # 开始刚开始并不是从这里进入的 read_module
    ...
    # 读取下一部分的长度，并判断是否有特殊编码
    # 返回 [length, if_is_encoded]
    def read_length_with_encoding(self, f):
        ...
        if enc_type == REDIS_RDB_ENCVAL:
            ...
            raise Exception('read_length_with_encoding',
                            "Invalid string encoding %s (encoding byte 0x%X)" % (enc_type, bytes[0]))
    ...
    def read_object(self, f, enc_type):
        if enc_type == REDIS_RDB_TYPE_STRING:
            ...
            self._callback.set(self._key, val, self._expiry,
                               info={'encoding': 'string', 'idle': self._idle, 'freq': self._freq})
            ...
            self._callback.start_list(self._key, self._expiry,
                                      info={'encoding': 'linkedlist', 'idle': self._idle, 'freq': self._freq})
            ...
            self._callback.start_set(self._key, length, self._expiry,
                                     info={'encoding': 'hashtable', 'idle': self._idle, 'freq': self._freq})
            for count in range(0, length):
                val = self.read_string(f)
            ...
            self._callback.start_sorted_set(self._key, length, self._expiry,
                                            info={'encoding': 'skiplist', 'idle': self._idle, 'freq': self._freq})
            for count in range(0, length):
                val = self.read_string(f)
    ...
    def read_intset(self, f):
        ...
        self._callback.start_set(self._key, num_entries, self._expiry,
                                 info={'encoding': 'intset', 'sizeof_value': len(raw_string), 'idle': self._idle,
                                       'freq': self._freq})
    ...
    def read_ziplist(self, f):
        ...
        self._callback.start_list(self._key, self._expiry,
                                  info={'encoding': 'ziplist', 'sizeof_value': len(raw_string), 'idle': self._idle,
                                        'freq': self._freq})
        for x in range(0, num_entries):
    ...
    def read_list_from_quicklist(self, f):
        ...
        total_size = 0
        self._callback.start_list(self._key, self._expiry,
                                  info={'encoding': 'quicklist', 'zips': count, 'idle': self._idle, 'freq': self._freq})
    ...
    def read_zset_from_ziplist(self, f):
        ...
        num_entries = read_unsigned_short(buff)
        if (num_entries % 2):
            raise Exception('read_zset_from_ziplist',
                            "Expected even number of elements, but found %d for key %s" % (num_entries, self._key))
        num_entries = num_entries // 2
        self._callback.start_sorted_set(self._key, num_entries, self._expiry,
                                        info={'encoding': 'ziplist', 'sizeof_value': len(raw_string),
    ...
    def read_hash_from_ziplist(self, f):
        ...
        num_entries = read_unsigned_short(buff)
        if (num_entries % 2):
            raise Exception('read_hash_from_ziplist',
                            "Expected even number of elements, but found %d for key %s" % (num_entries, self._key))
        num_entries = num_entries // 2
    ...
    def read_zipmap(self, f):
        ...
        self._callback.start_hash(self._key, num_entries, self._expiry,
                                  info={'encoding': 'zipmap', 'sizeof_value': len(raw_string), 'idle': self._idle,
                                        'freq': self._freq})
        while True:
    ...
    def matches_filter(self, db_number, key=None, data_type=None):
        ...
        if data_type is not None and 'types' in self._filters and (
        not self.get_logical_type(data_type) in self._filters['types']):
            return False
        return True
    ...
    def lzf_decompress(self, compressed, expected_length):
        if HAS_PYTHON_LZF:
            ...
            if len(out_stream) != expected_length:
                raise Exception('lzf_decompress', 'Expected lengths do not match %d != %d for key %s' % (
